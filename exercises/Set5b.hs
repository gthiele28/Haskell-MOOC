...
valAtRoot (Node val l r) = Just val
...
treeMax Empty = 0
treeMax (Node curr l r) = max curr (max (treeMax l) (treeMax r))
...
allValues condition (Node curr l r) = (condition curr) && (allValues condition l) && (allValues condition r)
...
mapTree f (Node curr l r) = Node (f curr) (mapTree f l) (mapTree f r)
...
cull val (Node curr l r)
  | (curr == val) = Empty
  | otherwise = Node curr (cull val l) (cull val r)
...
walk [] (Node curr l r) = Just curr
walk (StepL:steps) (Node curr l r) = walk steps l
walk (StepR:steps) (Node curr l r) = walk steps r
...
set (StepL:ps) val (Node x l r) = Node x (set ps val l) r
set (StepR:ps) val (Node x l r) = Node x l (set ps val r)
...
search target (Node x l r)
    | target == x = Just []
    | otherwise =
        case search target l of Just path -> Just (StepL : path)
                                Nothing -> case search target r of Just path -> Just (StepR : path)
                                                                   Nothing   -> Nothing
