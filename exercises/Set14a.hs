...
greetText input
    | (T.length input) > 15 = (T.pack "Hello, ") <> (T.take 15 input) <> (T.pack "...!")
    | otherwise = (T.pack "Hello, ") <> input <> (T.pack "!")
...
longestRepeat = longest . T.unpack
  where
    longest [] = 0
    longest (x:xs) = go x 1 1 xs

    go _ curr best [] = max curr best
    go prev curr best (x:xs)
      | x == prev = go x (curr + 1) best xs
      | otherwise = go x 1 (max curr best) xs
...
countUtf8Chars bs = either (const Nothing) (Just . T.length) (decodeUtf8' bs)
