...
appendAll ref strs = do val <- readIORef ref
                        writeIORef ref (val ++ (concat strs))
                        return ()
...
swapIORefs ref1 ref2 = do val1 <- readIORef ref1
                          val2 <- readIORef ref2
...
compose op1 op2 c = do val1 <- op2 c
                       val2 <- op1 val1
                       return val2
