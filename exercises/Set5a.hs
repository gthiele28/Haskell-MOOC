...
totalPrice (MkShoppingEntry name price quantity) = price * (fromIntegral quantity)
...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix c1 c2) = [(((rgb c1) !! 0) + ((rgb c2) !! 0)) / 2,
                   (((rgb c1) !! 1) + ((rgb c2) !! 1)) / 2,
                   (((rgb c1) !! 2) + ((rgb c2) !! 2)) / 2]
rgb (Invert c) = [ 1 - ((rgb c) !! 0), 1 - ((rgb c) !! 1), 1 - ((rgb c) !! 2) ]
...
toList Empty = []
toList (Pair key value xs) = (key, value) : toList xs

fromList :: [(k,v)] -> KeyVals k v
fromList [] = Empty
fromList ((key, value):xs) = (Pair key value (fromList xs))
...
fromBin :: Bin -> Int
fromBin End = 0
fromBin (O xs) = 2 * (fromBin xs)
fromBin (I xs) = 1 + (2 * (fromBin xs)) 
