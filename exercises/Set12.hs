...
instance Functor List where
  fmap f (Empty) = Empty
  fmap f (LNode val next) = LNode (f val) (fmap f next)
...
instance Functor TwoList where
  ...
  fmap f (TwoNode x1 x2 next) = TwoNode (f x1) (f x2) (fmap f next)
...
inBoth fold1 fold2 = [x | x <- toList fold1, x `elem` toList fold2]
