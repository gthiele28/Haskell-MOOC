...
-- Error and Finished are absorbing states
step Error _ = Error
...
-- Eggs added
step EggsAdded AddFlour = EggsFlour
step EggsAdded AddSugar = EggsSugar
step EggsAdded _        = Error

-- Eggs + flour
step EggsFlour AddSugar = ReadyToMix
step EggsFlour _        = Error

-- Eggs + sugar
step EggsSugar AddFlour = ReadyToMix
...
average (x :| xs) =
    (x + sum xs) / fromIntegral (1 + length xs)
...
passwordAllowed password (MinimumLength n) =
    length password >= n

passwordAllowed password (ContainsSome chars) =
    any (`elem` chars) password

passwordAllowed password (DoesNotContain chars) =
    all (`notElem` chars) password

passwordAllowed password (And r1 r2) =
    passwordAllowed password r1 &&
    passwordAllowed password r2

passwordAllowed password (Or r1 r2) =
    passwordAllowed password r1 ||
    passwordAllowed password r2
