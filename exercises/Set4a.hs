...
allEqual [] = True
allEqual xs = allEqual' xs (xs !! 0)

allEqual' :: Eq a => [a] -> a -> Bool
allEqual' [] _ = True
allEqual' (x:xs) e = if (x == e) then allEqual' xs e
                               else False
...
rangeOf xs = ((sort xs) !! ((length xs) - 1)) - ((sort xs) !! 0)
...
longest' [] long = long
longest' (x:xs) long
    | (length x) > (length long) = longest' xs x
    | (length x) == (length long) = if (x !! 0) < (long !! 0) then longest' xs x
                                                              else longest' xs long
    | otherwise = longest' xs long
...
average' [] sum len = sum / (fromIntegral len)
average' (x:xs) sum len = average' xs (sum + x) (len + 1)
...
winner scores player1 player2 = if (Map.findWithDefault 0 player1 scores) >= (Map.findWithDefault 0 player2 scores) then player1
                                                                                                                    else player2
...
freqs xs = freq' xs Map.empty

freq' [] map = map
freq' (x:xs) map = case (Map.lookup x map) of Nothing -> freq' xs (Map.insert x 1 map)
                                              (Just n) -> freq' xs (Map.adjust (\n -> n + 1) x map)
...
transfer from to amount bank = if (Map.member from bank) 
                               && (Map.member to bank) 
                               && (amount > 0) 
                               && ((Map.findWithDefault (-1) from bank) >= amount) 
                                    then Map.adjust (\x -> x + amount) to (Map.adjust (\x -> x - amount) from bank)
                                    else bank
...
swap i j arr = let iVal = arr ! i
                   jVal = arr ! j
               in arr // [(i, jVal), (j, iVal)]
