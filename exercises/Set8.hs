...
combine colorFunc (Picture f1) (Picture f2) =
    Picture (\coord -> colorFunc (f1 coord) (f2 coord))
...
union (Shape f) (Shape g) =
    Shape (\coord -> f coord || g coord)

cut :: Shape -> Shape -> Shape
cut (Shape f) (Shape g) =
    Shape (\coord -> f coord && not (g coord))
...
paintSolid color (Shape inside) (Picture base) =
    Picture (\coord ->
        if inside coord
           then color
           else base coord)
...
paint (Picture pat) (Shape inside) (Picture base) =
    Picture f
  where
    f coord
      | inside coord = pat coord
...
instance Transform Zoom where
    apply (Zoom n) (Picture pic) =
        Picture (\(Coord x y) ->
            pic (Coord (x `div` n) (y `div` n)))

data Flip = FlipX | FlipY | FlipXY
  deriving Show

instance Transform Flip where
    apply FlipX (Picture pic) =
        Picture (\(Coord x y) ->
            pic (Coord (-x) y))

    apply FlipY (Picture pic) =
        Picture (\(Coord x y) ->
            pic (Coord x (-y)))

    apply FlipXY (Picture pic) =
        Picture (\(Coord x y) ->
            pic (Coord y x))
...
instance Transform Blur where
    apply Blur (Picture pic) =
        Picture (\(Coord x y) ->
            let Color r1 g1 b1 = pic (Coord x y)
                Color r2 g2 b2 = pic (Coord (x-1) y)
                Color r3 g3 b3 = pic (Coord (x+1) y)
                Color r4 g4 b4 = pic (Coord x (y-1))
                Color r5 g5 b5 = pic (Coord x (y+1))
...
instance Transform BlurMany where
    ...
    apply (BlurMany n) pic = apply Blur (apply (BlurMany (n-1)) pic)
