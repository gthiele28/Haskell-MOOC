...
sorted :: [Int] -> Bool
sorted (x:y:xs) = if x <= y then sorted (y:xs)
                            else False
sorted (x:[]) = True
sorted [] = True
...
sumsOf :: [Int] -> [Int]
sumsOf []     = []
sumsOf (x:xs) = x : addToAll x (sumsOf xs)
...
mymaximum bigger initial (x : xs) = if (bigger x initial) then mymaximum bigger x xs
