...
prettyPrint size queens =
    concatMap makeRow [1..size]
  where
    makeRow r = [square (r,c) | c <- [1..size]] ++ "\n"
...
sameAntidiag (i,j) (k,l) = (k - i) ==  (0 - (l - j))
...
danger (x, y) [] = False
danger loc (queen:xs)
    | (sameRow loc queen) || (sameCol loc queen) || (sameDiag loc queen) || (sameAntidiag loc queen) = True
    | otherwise = danger loc xs
...
fixFirst n ((r,c):qs)
    | c > n = Nothing
    | not (danger (r,c) qs) = Just ((r,c):qs)
    | otherwise = fixFirst n ((r,c+1):qs)
