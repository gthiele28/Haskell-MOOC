...
firstHalf xs = take (((length xs) `div` 2) + ((length xs) `mod` 2)) xs
...
powers :: Int -> Int -> [Int]
powers k max = powers' [] k 1 max

powers' xs k n max
    | n > max = xs
    | otherwise = powers' (xs ++ [n]) k (n * k) max
------------------------------------------------------------------------------
...
whileRight check x = case (check x) of (Left val) -> val
                                       (Right val) -> whileRight check val
...
joinToLength l xs = [(x ++ y) | x <- xs, y <- xs, (length (x ++ y)) == l]
...
interpreter commands = interpreter' commands 0 0 [] 0

interpreter' :: [String] -> Int -> Int -> [String] -> Int -> [String]
interpreter' commands x y outs i
    | (length commands) <= i = outs
    | (commands !! i) == "printX" = interpreter' commands x y (outs ++ [show x]) (i + 1)
    | (commands !! i) == "printY" = interpreter' commands x y (outs ++ [show y]) (i + 1)
    | (commands !! i) == "up" = interpreter' commands x (y + 1) outs (i + 1)
    | (commands !! i) == "down" = interpreter' commands x (y - 1) outs (i + 1)
    | (commands !! i) == "left" = interpreter' commands (x - 1) y outs (i + 1)
    | (commands !! i) == "right" = interpreter' commands (x + 1) y outs (i + 1)